                ...
                <tr>
                  <td colspan="2"><table style="width:7.4cm;height:0.8cm;">
                    <tr>
                      ...
                      <td style="font-size:13px;font-family:Adobe Gothic Std B;" align="right"><b>AIR MAIL&nbsp;&nbsp;&nbsp;</b></td>
                    </tr>
                  </table></td>
                </tr>
                <tr>
                  <td valign="bottom"><table align="left" style="border:2px solid #000000;text-align:center;font-weight:bold;width:0.9cm;height:1.9cm;" cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="border-bottom:2px #000000 solid;"><s:if test="@kyle.leis.es.price.zone.dax.ZoneDemand@getZnvnameByDistrict(@kyle.leis.es.price.zone.dax.ZoneDemand@getZfCodeByCity('SG_NR'), #bean.Cwdt_code_signin)==null"> &nbsp; </s:if>
                            <s:elseif test="(@kyle.leis.es.price.zone.dax.ZoneDemand@getZnvnameByDistrict(@kyle.leis.es.price.zone.dax.ZoneDemand@getZfCodeByCity('SG_NR'), #bean.Cwdt_code_signin)).equals('')">&nbsp; </s:elseif>
                            <s:else>
                              <s:property value="@kyle.leis.es.price.zone.dax.ZoneDemand@getZnvnameByDistrict(@kyle.leis.es.price.zone.dax.ZoneDemand@getZfCodeByCity('SG_NR'), #bean.Cwdt_code_signin)" />
                            </s:else>                        </td>
                      </tr>
                      <tr>
                        <td style="border-top:2px #000000 solid;font-family:Adobe Gothic Std B;">NR</td>
                      </tr>
                  </table></td>
                  <td valign="bottom" align="right"><table cellpadding="0" cellspacing="0" style="border:2px solid #000000; font-weight:bold; width:6.4cm; height:1.9cm;">
                      <tr>
                        <td align="center" style="font-size:12px;font-family:rial, Helvetica, sans-serif;"> SINGEAPORE </td>
                      </tr>
                      <tr>
...
<tr><td style="font-size:13px;" colspan="3" align="left">
	...
	<s:if test="#request.listWayBillsSWGH.size()==0 && #request.listWayBillsOTSR.size()==0">
		<s:if test="#request.listWayBillsBQ.size()==0 && #request.listWayBillsOTS.size()==0 && #request.listWayBillsPY.size()==0 && #request.listWayBillsGH.size()==0 && #request.listWayBillsSGGH.size()==0">
			<s:if test="(#pyindex.index+1)%6==0 && (#pyindex.index+1) != (#request.listWayBillsSGPY.size())">
			  	<p style="page-break-after:always">&nbsp;</p>
			 </s:if>
		</s:if>	
		<s:else>
			<s:if test="(#pyindex.index+1+(6-(#request.listWayBillsBQ.size()+#request.listWayBillsOTS.size()+#request.listWayBillsPY.size()+#request.listWayBillsGH.size()+#request.listWayBillsSGGH.size())%6))%6==0 && (#pyindex.index+1) != (#request.listWayBillsSGPY.size())">
			  	<p style="page-break-after:always">&nbsp;</p>
			 </s:if>
		</s:else>		
	</s:if>
	<s:else>
		<s:if test="#request.listWayBillsBQ.size()==0 && #request.listWayBillsOTS.size()==0 &&  #request.listWayBillsPY.size()==0 && #request.listWayBillsGH.size()==0 && #request.listWayBillsSGGH.size()==0">
			<s:if test="(#pyindex.index+1)%6==0">
			  	<p style="page-break-after:always">&nbsp;</p>
			 </s:if>
		</s:if>	
		<s:else>
			<s:if test="(#ghindex.index+1+(6-(#request.listWayBillsBQ.size()+#request.listWayBillsOTS.size()+#request.listWayBillsPY.size()+#request.listWayBillsGH.size()+#request.listWayBillsSGGH.size())%6))%6==0">
			  	<p style="page-break-after:always">&nbsp;</p>
			 </s:if>
		</s:else>	
	</s:else>		
