...
<title>修改新闻页面</title>
	...
	function onEditorBlur() {
	 ...
	 function checkHeading(){
			...
			var strBlId = document.getElementById("strBlId").value;
			if(blHeading==""){
				return;
			}
			var msg = "";
			$.ajax({
				url : 'headingAjax',
				type : 'post',
				data : {blHeading : blHeading,strBlId : strBlId},
				async: false,
				cache : false,			
				timeout : 60000,
	...
	function checkBulletinHeadingNotNull(){
		var blHeading = document.saveform.bulletinHeading;
		if(blHeading.value.length==0||blHeading.value==""){
			alert("请输入公告标题");
			blHeading.focus();
			return false;
		}
...
<body>
	&quot;<!-- 设置文章 -->			
	<s:set var="bulletinColumns" value="#request.bulletinColumns" />
<form action=""
	method="post" name="saveform" id="saveform" >
	<input type="hidden" name="strBlId" id="strBlId" value="<s:property value='#bulletinColumns.Blblid' />" />
	<input type="hidden" name="modifyflag" value="true" />
	<input type="hidden" name="strOperId" value="<s:property value='#bulletinColumns.Copopid' />" />
    <div id="admin">
	    <div id="top"><img src="<%=path%>/m_images/logo.jpg" /><span> 您现在的位置<a href="#">首页---</a>后台管理---修改公告</span>
	</div>
		   <div class="master">
					...
					<table width="730px" border="0" cellpadding="0" cellspacing="1">
						<thead>
							...
							<tr>
								...
								<td colspan="3">
									<input name="bulletinHeading" type="text" class="input_text" id="bulletinHeading" style="width: 592px" value="<s:property value="#bulletinColumns.Blblheading" />" />
							...
							<tr>
								...
								<td width="80px">
									<select name="bulletinType" id="bulletinType" style="width:245px; ">
										...
										<s:iterator value="#bkList">
											<s:if test="Bkbkcode.equals(#request.bulletinColumns.Bkbkcode)">
												<option value='<s:property value="Bkbkcode"/>' selected="selected">
								...
								<td>
									<input name="blvaliddate" type="text" class="input_text"
										style="width: 210px"
										value="<s:property value="#bulletinColumns.Blblvaliddate" />"
							...
							<tr>
								...
								<td width="80px">
									<select name="bulletinLevel" style="width:248px; ">
										<s:if test='#bulletinColumns.Bllblcode.equals("001")'>
											<option value="001" selected="selected">内部公告</option>
											<option value="002">外部公告</option>
										</s:if>
										<s:elseif test='#bulletinColumns.Bllblcode.equals("002")'>
											<option value="001">内部公告</option>
											<option value="002" selected="selected">外部公告</option>
										</s:elseif>
									</select>									
								</td>
							</tr>
							 ...
							 <tr>
								<td colspan="4">
									<textarea rows="140"  name="bulletincontent" id="bulletincontent" >
										<s:property value="#bulletinColumns.Blblcontent" />
									</textarea>
								</td>
							</tr>
						</table>	
