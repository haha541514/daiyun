...
//运费测算
function costbudget(){
   var weight=$("#text1").val();
   var dt_hucode=$("#C_countryCode").val();
   ...
   //包裹类型
   var ctp=$("#ctp input:radio:checked").val();
   $("#ctcode").val(ctp);
   $("#cosmsg").html("运费计算中..");
   var formData = new FormData($("#predictOrderForm")[0]);
   $.ajax({url:'<%=path%>/costBudgetAjax',
   type:'post',
   data:formData,
   async:true,
   cache:false,
...
document.body.style.overflow = "hidden"; //取消滚动
//以下部分实现弹出层的拖拽效果
var posX;
var posY;
...
$(document).ready(function(){   
	...
	var ckckcode=$("#ckckcode").val();
	var index=$("#hw_table tbody").children().length;
		$("#hw_table").append("<tr>" +
		"<td><input name='hw_index' type='text'  value='"+index+"'style='width:50px;text-align: center;border:0px'/></td>"+
           "<td><input  name='ciciename' type='text'  value='' style='width:110px;text-align: center; border:0px'/></td>"+
           "<td><input  name='ciciname'     style='width:110px;text-align: center; border:0px'/></td>"+        
           "<td><input name='ciciunitprice' id='ciciunitprice'  type='text'     onkeyup='clearNoNum(this)' style='width:140px;text-align: center; border:0px'/></td>"+
          "<td><input name='ckckcode' id='ckckcode' type='text' value='"+ckckcode+"'  style='width:120px;text-align: center; border:0px'/></td>"+
           "<td><input name='cicipieces' id='cicipieces'   onkeyup='clearNoNum(this)' style='width:70px;text-align: center; border:0px'/></td>"+
          "<td><input name='money' id='money' type='text' style='width:110px;text-align: center; border:0px'/></td>"+
		...
		$("#hw_table [name='hw_de']").on("click", function(){
			...
			var a = 2;
			$("#hw_table [name='hw_index']").each(function(){
				$(this).attr("value",a);
				a=a+1;
			});
			});	
...
//选择币种
function choseCurrency(){

var currency=$("#currency option").val();
var currency2=$("#currency").find("option:selected").val();
$('[name="ckckcode"]').val(currency2);
}
/*
//计算金额
function calculateMoney(){
  var count=$("#hw_table tbody").children().length-1;
//  alert("值："+$("#cicipieces").eq(0).val());
  for(var i=0;i<count;i++){
  var pieces=$("#cicipieces").eq(i).val();
  var price=$("#ciciunitprice").eq(i).val();
  var money=pieces*price;
  $("#money").eq(i).val(money);
  }
}*/
//根据货件信息修改客户重量
function changeweight(){
var totalweight=0;
 $("#hj_table [name='cpCpgrossweight']").each(function(){
   totalweight+=parseInt($(this).val());
  });
   $("#text1").val(totalweight);
   //试算运费
   costbudget();
}
...
$(document).ready(function(){
	...
	$("#pieces_show").val(index);
		$("#hj_table").append("<tr>" +
            ...
            "<td><input  name='cpCpgrossweight' onchange='changeweight()'  type='text' onkeyup='clearNoNum(this)'    style='text-align: center; border:0px'/></td>"+
            ...
            "<td id='hj_op' style='width:110px;font-size:8px'><a name='hj_edit'id='hj_edit' href='javaScript:void(0);'>保存</a><a name='hj_de' id='hj_de' href='javaScript:void(0);'>删除</a>"+
"</td></tr>");
		   //删除
...
<div id="main">
  ...
  <div class="forwarding">
    ...
    <div class="right" style="font-size:12px;">
      ...
      <div class="home">
       <h3><a href="#">我的代运</a> > 我的订单 > <span>新建订单</span></h3>
      </div>
      <div class="order" >
	...
	<input type="hidden" id="cw_code"  value="<s:property value='#obean.Cwcw_code'/>" name="cw_code"/>
        ...
        <div class="nir">
          <table width="818" border="0" id="basic_information" style="font:12px '宋体';">
            <tr>
              ...
              <td style="padding-left:40px;" >物流渠道：


                <select onchange="costbudget()" id="pkCode" name="objWaybillforpredictColumns.pkpk_code" style="width:300px;height:26px">                
                  <option value="">-----------------请选择物流渠道-----------------</option>
                  <s:iterator var="products" value="@com.daiyun.dax.ProductkindDemand@getAllProduct()">
                        <s:if test="#obean.Pkpk_code.equals(#products.Pkcode) || #obean.Pkpk_code.equals(#request.salebean_Pkcode) ">
            ...
            <tr>
              <td>起运城市：
               ...
               <select onchange="costbudget()"  name="objWaybillforpredictColumns.cwDt_code_origin" style="width:100px;height:26px;margin-left:0px;">
               <s:iterator var="scity" value="@com.daiyun.dax.DistrictqueryDemand@queryStartDistrict()">
                  <s:if test="#obean.cwDt_code_origin.equals(#scity.diDthubcode) || #obean.cwDt_code_origin.equals(#request.OrginDtcode)">
                   <option  value="<s:property value='#scity.dtDtcode'/>" style="width:110px;height:20px;" selected="selected">
                   <s:property value='#scity.diDtname'/>
                    </option>
                  </s:if>
                   <s:elseif test='#scity.diDtcode.equals("719")&&#obean==null'>
               <option value="<s:property value='#scity.diDtcode' />" selected="selected" style="width:110px;height:20px;"><s:property value='#scity.diDtname'/></option>
               </s:elseif>
                  <s:else>
               <option value="<s:property value='#scity.diDtcode' />" style="width:110px;height:20px;"><s:property value='#scity.diDtname'/></option>
               </s:else>
               </s:iterator>
               </select>
             &nbsp;&nbsp; 实重：<input id="text1" onchange="costbudget()" name="objWaybillforpredictColumns.cwcw_customerchargeweight" value="<s:property value='#obean.Cwcw_customerchargeweight'/><s:property value='#request.salebean_Chargeweight'/>" onblur="isCheck('text1','5','实重',1,'text1')"   type="text" style="width:98px; height:26px" /><span id ="msgtext1" style="color:red; " >&nbsp;kg</span></td>
              ...
              <td id="ctp" style="font:12px '宋体';padding-left:40px; ">包裹类型：




               <s:iterator var="products" value="@com.daiyun.dax.CargoTypeDemand@queryAllCargotypes()">
               <s:if test='#products.ctctcode.equals("AWPX")'>
               <input onchange="costbudget();" id="Package_type_0" name="objWaybillforpredictColumns." value="<s:property value='#products.ctctcode'/>" checked="checked" type="radio">
                <label for="Package_type_0" style="padding-right:30px;"><s:property value='#products.ctctname'/></label>
               </s:if>
               <s:else>
                <input onchange="costbudget();" id="Package_type_0" name="objWaybillforpredictColumns." value="<s:property value='#products.ctctcode'/>" type="radio">
                <label for="Package_type_0" style="padding-right:30px;"><s:property value='#products.ctctname'/></label>
                </s:else>
                 </s:iterator>
                 <input type="hidden" id="ctcode" name="ctcode" value="">
                </td>
            </tr>
            <tr>
              ...
              <!-- <td align="right"><span style="padding-left:24px; padding-right:30px;">实重：
               ...
               <s:if test="#obean.cwcw_pieces!=null">
                ...
                <input id="pieces_show" name="objWaybillforpredictColumns.cwcw_pieces" value="1" readonly="readonly"   type="text" style="width:95px;  background-color: #f1f1f1 " />
                  ...
                  <td><span style="padding-left:40px; padding-right:30px;">参考运费：
                <input  id="freight"  type="text" readonly="readonly" style="width:100px; margin-right:5px;background-color: #f1f1f1" value="<s:property value='#request.salebean_Freightvalue'/>"/>
                元</span> <label id="cosmsg" style="color:red"></label><!--  件数：
            ...
            <table width="393" border="0" id="recipient" style="font:12px '宋体';">
              ...
              <tr >
                <td style="padding-left:6px;">&nbsp;国家：     
                 <select  id="S_countryCode" name="objWaybillforpredictColumns.hwDt_code_shipper" style="width:134px;height: 20px;margin-left:0px;" onchange="changeHubcodeShipper()">
                      <option value="">----请选择国家---</option>
                      <s:iterator var="country" value="@com.daiyun.dax.CountryDemand@queryAllCountry()">
                        <s:if test="#obean.hwDt_code_shipper.equals(#country.dtDt_hubcode)">
              ...
              <tr >
                ...
                <s:else>
                  <input type="text" id="S_Dtdt_hubcode" name="Dtdt_hubcode"    style="width:135px;margin-left:0px;"  value="<s:property value='#obean.hwDt_code_shipper'/>" onchange="changeCountryShipper()" />
                  </s:else>
                  </td>
            ...
            <table width="393" border="0" id="recipient" style="font:12px '宋体';">
              ...
              <tr>
                <td style="padding-left:6px;">&nbsp;国家：
                 ...
                 <input type="text"   id="C_destination_2" style="width:135px"  value="<s:property value='#obean.Dtdt_name'/><s:property value='#request.destination_2'/>" onclick="C_setLeft(this)"> 
...
function Assignment(obj){
	...
	costbudget();	
              ...
              <tr>
                <td>二字码：
                  <input name="province" id="C_Dtdt_hubcode" onclick="C_setLeft(this)" value="<s:property value='#obean.Dtdt_hubcode'/><s:property value='#request.Dtdt_hubcode'/>" type="text" style="width:135px;margin-left:0px;"  onchange="changeCountryConsign()"/></td>                  
                <td style="padding-left:6px;">城市：
      ...
      <div class="goods">
        ...
        <table id="hj_table" width="860" id="goods" style="font:12px '宋体';">
          <tr style="background:#edf8fd;">
           ...
           <s:if test="#request.listPieces != null">
				...
				<tr>
					...
					<td width="15%"><input name="cpCpgrossweight" onchange="changeweight()" type="text"  onkeyup="clearNoNum(this)"  style="text-align: center; border:0px" value="<s:property value='#beans.Cpcpgrossweight'/>"/></td><!-- 每件的实重不计。 -->
					<td width="15%" ><input name="cpCplength"   type="text" onkeyup="clearNoNum(this)" style="border:0px;width:170px;text-align:center;" name="length1" value="<s:property value='#beans.cpCplength'/>"></td>
					<td width="15%" ><input name="cpCpwidth"  type="text" onkeyup="clearNoNum(this)" style="border:0px;width:170px;text-align:center;" name="width1" value="<s:property value='#beans.cpCpwidth'/>"></td>
					<td width="15%" ><input name="cpCpheight"  type="text" onkeyup="clearNoNum(this)" style="border:0px;width:170px;text-align:center;" name="height1" value="<s:property value='#beans.cpCpheight'/>"></td>
          ...
          <tr>
            ...
            <td width="15%"><input name="cpCpgrossweight" onchange="changeweight()" type="text"  onkeyup="clearNoNum(this)"  style="text-align: center; border:0px" /></td>
          ...
          <tr>
            ...
            <td width="15%"><input name="cpCpgrossweight" onchange="changeweight()" value="<s:property value='#listpic.cpcpgrossweight'/>" onkeyup="clearNoNum(this)" type="text"   style="text-align: center; border:0px"/></td>
            ...
            <td width="15%" id="hj_op"><a name="hj_edit" id='hj_edit' href="javaScript:void(0);">保存</a><a id='hj_de' href='javaScript:void(0);'>删除</a></td>
              ...
              <s:if test="#index.index!=0">
               <tr>
            <td width="10%"><input name="hj_index" value="<s:property value='#index.index+1'/>"  style="width:50px;text-align: center;border:0px" name="edit" type="text" readonly="readonly"  ></td>
            <td width="15%"><input name="cpCpgrossweight" onchange="changeweight()" value="<s:property value='#listpic2.cpcpgrossweight'/>" onkeyup="clearNoNum(this)" type="text"  style="text-align: center; border:0px"/></td>
      ...
      <div class="goods">
        ...
        <table id="hw_table" width="860" id="goods" style="font:12px '宋体';">
          ...
          <td>序号</td> 
            ...
            <td width="10%">单价</td>
            <td width="10%">币种
            <select name="currency" id="currency" onchange="choseCurrency();">
           <option  value="USD">美金</option>
            <option  value="RMB">人民币</option>
            <option value="HKD">港币</option>
            <option value="KRW">韩币</option>
            ...
            <td width="10%">数量</td>
            <td width="10%">金额</td>
           <td style="width:10px;text-align: center;border:0px">海关/产品编码</td>
            <td width="10%">操作</td>
          </tr>
          <s:set var="beans" value="#request.listCargoinfoColumns" />
            <s:if test="#beans==null">
          <tr>  
            <td width="10%"><input name="hw_index" id="hw_index" value="1" style="width:50px;text-align: center;border:0px" name="edit" type="text" readonly="readonly"  /></td>
          <td><input name="ciciename" value="<s:property value='#listCargoinfo.ciciename'/>" type="text"  value="shoes" style="width:110px;text-align: center; border:0px"/></td>
            <td><input name="ciciname" onchange="costbudget();" value="<s:property value='#listCargoinfo.ciciname'/>" type="text"   style="width:110px;text-align: center; border:0px"  /> </td>            
            <td><input name="ciciunitprice" id="ciciunitprice" type="text"  value="<s:property value='#listCargoinfo.Ciciunitprice'/>" onkeyup="clearNoNum(this)" style="width:140px;text-align: center; border:0px"/></td>
          <td><input id="ckckcode" name="ckckcode" type="text"  value="<s:property value='#listCargoinfo.ckCkcode'/>"  style="width:120px;text-align: center; border:0px"/></td>
            <td><input name="cicipieces" id="cicipieces" type="text"  value="<s:property value='#listCargoinfo.Cicipieces'/>"  onkeyup="value=value.replace(/[^\d]/g,'') " onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"  style="width:70px;text-align: center; border:0px"/></td>
          <td><input name="money" id="money" value="" type="text" style="width:110px;text-align: center; border:0px"/>
          <s:property value="#listCargoinfo.Ciciunitprice*#listCargoinfo.Cicipieces"/>
           </td>
            ...
            <td id="hw_op" style="width:110px;font-size:8px"><a id="hw_edit" name='hw_edit' href="javaScript:void(0);">保存</a><a name='hw_de' href="javaScript:void(0);">删除</a></td>
          ...
          <td><input name="ciciename" value="<s:property value='#listCargoinfo.ciciename'/>" type="text"  value="shoes" style="width:110px;text-align: center; border:0px"/></td>
           ...
           <td><input name="cicipieces" id="cicipieces" type="text"  value="<s:property value='#listCargoinfo.Cicipieces'/>"  onkeyup="value=value.replace(/[^\d]/g,'') " onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"  style="width:70px;text-align: center; border:0px"/></td>
          <td><input name="money" id="money" value="" type="text" style="width:110px;text-align: center; border:0px"/>   
     <!--<s:property value='#listCargoinfo.Ciciunitprice'/>*<s:property value='#listCargoinfo.Cicipieces'/>        
         -->
         <s:property value="#listCargoinfo.Ciciunitprice*#listCargoinfo.Cicipieces"/>
          </td>
         <td><input name="ciciattacheinfo" type="text"  value="<s:property value='#listCargoinfo.Ciciattacheinfo'/>" style="width:50px;text-align: center;border:0px"/></td>
     ...
     <s:iterator var="list" value="#request.listCargoinfoColumns" status="index">
            <s:if test="#index.index!=0">
            <tr>
            <td ><input name="hw_index" id="hw_index" style="width:50px;text-align: center;border:0px" value="<s:property value='#index.count'/>"/></td>           
           <td><input name="ciciename" value="<s:property value='#listCargoinfo.ciciename'/>" type="text"  value="shoes" style="width:110px; text-align: center; border:0px"/></td>
            <td><input name="ciciname" onchange="costbudget();" value="<s:property value='#listCargoinfo.ciciname'/>" type="text"   style="width:110px;text-align: center; border:0px"  /> </td>   
            <td><input name="ciciunitprice" type="text"  value="<s:property value='#listCargoinfo.Ciciunitprice'/>" onkeyup="clearNoNum(this)" style="width:140px;text-align: center; border:0px"/></td>
           <td><input id="ckckcode" name="ckckcode" type="text"  value="<s:property value='#listCargoinfo.ckCkcode'/>" style="width:120px;text-align: center; border:0px"/></td>
            <td><input name="cicipieces" type="text"  value="<s:property value='#listCargoinfo.Cicipieces'/>"  onkeyup="value=value.replace(/[^\d]/g,'') " onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"  style="width:70px;text-align: center; border:0px"/></td>
           <td><input name="money" id="money" value="" type="text" style="width:110px;text-align: center; border:0px"/>
         <s:property value="#listCargoinfo.Ciciunitprice*#listCargoinfo.Cicipieces"/>
          </td>
            <td><input name="ciciattacheinfo" type="text"  value="<s:property value='#listCargoinfo.Ciciattacheinfo'/>" style="width:50px;text-align: center;border:0px"/></td>
      ...
      <div class="off" style="display:none" id="detail">
        ...
        <div class="nir">
          ...
          <s:iterator var="cargokind" value="@com.daiyun.dax.CargokindDemand@queryAll()"> 
                ...
                <s:elseif test='#cargokind.ckcgkcode.equals("1")' >
                <input id="SelType_1"  name="sel_Type" value="<s:property value='#cargokind.ckcgkcode'/>" type="radio" checked="checked" /> 
               <label for="SelType_0" style="padding-right:24px;"><s:property value="#cargokind.ckcgkname"/></label>     
                </s:elseif>
                <s:else>
           ...
           <s:iterator var="batterykind" value="@com.daiyun.dax.BatterykindDemand@getAllBatterykind()">     
               ...
               <s:else>
                ...
                <label for="Customs_declaration_1"   style="padding-right:24px;">否</label>
                 ...
                 <li id="dcuSign">是否关税预付:&nbsp;
         
                <s:if test='#obean.hwhw_dcustomssign=="Y"'>
                <input id="est_code_declaration_0" name="est_code"  value="Y" checked="checked" type="radio" />
                <label for="est_code_declaration_0" style="padding-right:24px;">是</label>
                </s:if>
                <s:else>
                 <input id="est_code_declaration_0" name="est_code"  value="Y" type="radio" />
                <label for="est_code_declaration_0" style="padding-right:24px;">是</label>
                </s:else>
                <s:if test='#obean.hwhw_dcustomssign=="N"'>
                <input id="est_code_declaration_1" name="est_code" value="N" checked="checked"  type="radio" /> 
                <label for="est_code_declaration_1"   style="padding-right:24px;">否</label>
                </s:if>
                <s:else>
                <input id="est_code_declaration_1" name="est_code"  value="N"   type="radio" /> 
                <label for="est_code_declaration_1"   style="padding-right:24px;">否</label>
                </s:else>
                </li>
                
               <!--  <li>参考运费：
                <input id="freight" name="freight" type="text"  style="width:100px; margin-right:5px;" />
                元&nbsp;&nbsp;&nbsp;<label id="cosmsg" style="color:red">*</label></li>  -->
              <!--  <li id="pmCode">付款方式：
      ...
      <div >
        <button class="Submit" type="button" onclick="sub()"></button>
      </div>
      </form>
    </div>
  </div>
