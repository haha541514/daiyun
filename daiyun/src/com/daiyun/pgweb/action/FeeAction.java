...
//					cc.setCpcpgrossweight(new BigDecimal("0"));
	...
	public String costBudgetIndex() throws Exception{
		//货物名称
		ProductkindDemand pkd=new ProductkindDemand();
		List objProductkind=pkd.queryProductkind2();
		ServletActionContext.getRequest().setAttribute("objProductkind", objProductkind);
...
//					cc.setCpcpgrossweight(new BigDecimal("0"));
			...
			//取到公司代码	
			String cocode=null;
			if(ServletActionContext.getRequest().getSession().getAttribute("Cocode")!=null){
				cocode=ServletActionContext.getRequest().getSession().getAttribute("Cocode").toString();
			}
			...
			ServletActionContext.getRequest().setAttribute("gmcode","1525");
